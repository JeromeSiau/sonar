...
class _RadarScreenState extends ConsumerState<RadarScreen> {
  ...
  Future<void> _onCelebrationComplete() async {
    if (mounted) {
      // Record that user found a device (may trigger review request after 3 finds)
      await ref.read(reviewServiceProvider).recordDeviceFound();
      if (mounted) {
        context.pop();
      }
    }
